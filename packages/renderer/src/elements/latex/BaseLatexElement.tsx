...
import type { PPTLatexElement } from '@shikshasetu/dsl';
      ...
      >
        <div
          ...
          style={{
            ...
            // (e.g. a blue weight term) so it isn't forced to the browser default.
...
function KatexContent({
  ...
  // first measure on a cold export runs against the fallback font, the brace ends
  // up sized wrong and the piecewise body (`cases`) desynchronizes from it.
  // Re-measuring after `document.fonts.ready` snaps the scale back to the real
  // glyph metrics before the off-screen snapshot captures the frame.
  const measure = useCallback(() => {
    ...
    if (naturalW > 0 && naturalH > 0) {
      ...
      // A short formula sitting in a large frame would otherwise get scaled up
      // to fill the box and render huge.
