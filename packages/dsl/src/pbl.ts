...
/** Tolerates unknown members on the project tree (older documents may carry app runtime fields). */
...
/** Persisted seat for an agent chat thread; `messages` contents are app-domain, not interpreted here. */
export interface PBLThreadSeat {
  agentId: string;
  messages: unknown[];
}

/** Design-time PBL definition plus the canonical planner-seeded initial skeleton. */
...
export interface PBLContent {
  ...
  /** Legacy pre-v2 payload, kept as a compatibility mirror alongside `projectV2`.
   * @deprecated
   */
