/**
 * Action: the universal playback verb contract, the sole mechanism for agents
 * to drive a presentation. Two categories: fire-and-forget visual effects
 * (spotlight, laser), and synchronous actions that must complete before the
 * next action runs (speech, whiteboard, video, discussion, widget interactions).
 */
...
export interface SpeechAction extends ActionBase {
  ...
  text: string;
  /**
   * Asset reference for narration audio. Legacy unconverted documents may still
   * carry TTS-derived ids here, resolved through the app's legacy read fallbacks.
   */
  audioId?: AssetRef;
...
// Compile-time check that every ActionType appears in ACTION_TYPES.
