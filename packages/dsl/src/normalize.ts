/**
 * Element normalization for the slide DSL contract: the companion to
 * `validate.ts`. Where validators *report* on a document, `normalize`
 * *repairs* one: fills required fields a producer left off, derives
 * geometry-dependent fields, and fails loud on values it cannot interpret.
 *
 * Missing required field -> canonical default; present but wrong-typed ->
 * throws; present and well-formed -> passed through untouched. Pure and
 * non-mutating; idempotent.
 *
 * Scope is element **content** (font/colour/fill/style/points, and geometry
 * it can derive like a line's `start`/`end` or a shape's `viewBox`/`path`).
 * It does NOT fill or check base identity/geometry (`id`, `left`, `top`,
 * `width`, `height`, `rotate`), which is producer-supplied; use `validate*`
 * for that.
 */
import type {
  PPTElement,
...
/** Canonical static defaults for required element fields; mirrored onto the generated
 * JSON Schema via `@default` JSDoc in `slides.ts` and pinned together by a test.
 * Geometry-derived defaults (line start/end, shape viewBox/path) are computed instead. */
...
function pblFail(field: string, expected: string, value: unknown): never {
  throw new Error(
    `@shikshasetu/dsl: cannot normalize PBL project: \`${field}\` must be ${expected}, got ${JSON.stringify(value)}`,
  );
}
...
/**
 * Normalize a complete design-authored PBL project. Design-authored fields
 * must be present (normalization cannot invent project content); seeded
 * skeleton fields may be absent and receive canonical values. Milestone and
 * microtask statuses must each be consistently absent or present across the
 * whole project (mixed presence throws); all-absent milestones seed
 * first-active then locked, all-absent microtasks seed `todo`.
 */
...
function fail(el: Raw, field: string, expected: string, value: unknown = el[field]): never {
  throw new Error(
    `@shikshasetu/dsl: cannot normalize ${String(el.type)} element ${JSON.stringify(el.id)}: ` +
      `\`${field}\` must be ${expected}, got ${JSON.stringify(value)}`,
  );
}

/** Fill a required string field; `undefined` and `''` both count as absent. */
...
export function normalizeElement(el: unknown): PPTElement {
  if (!isObject(el))
    throw new Error(
      `@shikshasetu/dsl: cannot normalize element: expected an object, got ${JSON.stringify(el)}`,
    );
  switch (el.type) {
    case 'text':
    ...
    default:
      throw new Error(
        `@shikshasetu/dsl: cannot normalize element ${JSON.stringify(el.id)}: ` +
