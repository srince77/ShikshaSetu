...
import { isWidgetType, normalizeElement } from '@shikshasetu/dsl';
 ...
 *
 * Trimmed to the single widget type this pipeline generates for the MVP:
 * every interactive scene is a `simulation` widget.
 */
function inferWidgetType(_subject: string, _concept: string, _designIdea: string): WidgetType {
  return 'simulation';
}
...
function buildWidgetOutline(
  ...
  if (widgetType === 'simulation') {
    const varMatch = config.designIdea.match(/variables|adjust|slider/i);
 ...
 * owned by the DSL contract — `normalizeElement` from `@shikshasetu/dsl` — rather
...
async function generateSlideContent(
  ...
  let assignedImagesText = 'No images available, do not insert any image element';
  ...
  if (outline.mediaGenerations && outline.mediaGenerations.length > 0) {
    ...
    if (mediaParts.length > 0) {
      ...
      if (assignedImagesText.includes('No images available')) {
 ...
 *
 * Simplified single-call generator (this pipeline does not port the
 * reference multi-stage planner/kernel): one prompt asks the model for the
 * project's title/description/role/milestones directly; this function then
 * seeds the remaining `PBLProject` runtime fields (status, threads,
 * timestamps, ...) that the platform, not the model, owns.
 */
...
interface GeneratedPBLProjectShape {
  title?: string;
  description?: string;
  ...
  milestones?: Array<{
    ...
    debrief?: string;
    microtasks?: Array<{ title?: string; description?: string; hints?: string[] }>;
  }>;
}
...
async function generatePBLSceneContent(
  ...
  const language = targetLanguage || userRequirements?.requirement || 'the classroom language';
  const milestoneCount = pblConfig.issueCount && pblConfig.issueCount > 0 ? pblConfig.issueCount : 4;

  const prompts = buildPrompt(PROMPT_IDS.PBL_CONTENT, {
    projectTopic: pblConfig.projectTopic || outline.title,
    projectDescription: pblConfig.projectDescription || outline.description,
    targetSkills: (pblConfig.targetSkills || []).join(', ') || outline.title,
    milestoneCount,
    proficiency: 'intermediate',
    courseContext: buildCourseContext({
      pageIndex: outline.order,
      totalPages: outline.order,
      allTitles: [outline.title],
      previousSpeeches: [],
    }),
    language,
    languageDirective: languageDirective || '',
  ...
  if (!prompts) {
    throw new PBLGenerationError(`PBL content prompt unavailable for "${outline.title}"`);
  }
  ...
  try {
    ...
    throw new PBLGenerationError(`PBL generation call failed for "${outline.title}"`, {
      cause: error,
    });
  }

  const parsed = parseJsonResponse<GeneratedPBLProjectShape>(response, { logger: log });
  if (!parsed || !parsed.title || !parsed.description || !Array.isArray(parsed.milestones)) {
    throw new PBLGenerationError(`Failed to parse PBL project response for "${outline.title}"`);
  }

  const now = new Date().toISOString();
  const roleId = nanoid();
  const proficiency: PBLProject['proficiency'] = (
    ['beginner', 'intermediate', 'advanced'] as const
  ).includes(parsed.proficiency as 'beginner' | 'intermediate' | 'advanced')
    ? (parsed.proficiency as PBLProject['proficiency'])
    : 'intermediate';
  ...
  const projectV2: PBLProject = {
    ...
    gains: parsed.gains || [],
    tags: parsed.tags && parsed.tags.length > 0 ? parsed.tags : pblConfig.targetSkills || [],
    language,
    languageDirective,
    schemaVersion: 2,
    proficiency,
    ...
    roles: [
      {
        id: roleId,
        type: 'instructor',
        name: parsed.role?.name || `${outline.title} Coach`,
        description: parsed.role?.description,
        systemPrompt: parsed.role?.systemPrompt,
      },
    ],
    milestones,
    submissions: [],
    evaluations: [],
    threads: [{ agentId: roleId, messages: [] }],
    engagementEvents: [],
    createdAt: now,
    updatedAt: now,
  };
  ...
  log.info(
    `PBL project generated: ${projectV2.milestones.length} milestones, ${projectV2.roles.length} role(s)`,
  );
...
export async function generateWidgetContent(
  ...
  // Select appropriate prompt based on widget type. The MVP pipeline
  // generates a single widget kind (`simulation`); the other DSL widget
  // types (diagram/code/game/visualization3d/procedural-skill) are valid
  // playback content but are not produced by this trimmed generator.
  ...
  switch (widgetType) {
    ...
    default:
      log.warn(`Unsupported widget type in this pipeline: ${widgetType}`);
      return null;
  }
  ...
  return [
    {
      ...
      text: "Let's begin this project-based learning activity: understand the driving problem, then explore and practice it step by step in the project workspace.",
    },
  ];
}
  ...
  return [
    {
      ...
      text: "Let's take a short quiz to check what you've learned so far.",
    },
  ];
}
  ...
  return [
    {
      ...
      text: "Let's explore this concept through an interactive visualization. Try manipulating the elements on the page and observe what changes.",
    },
  ];
}
