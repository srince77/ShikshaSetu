...
- Prefix or tag speech with a speaker name or label in parentheses. NEVER write things like `(Teaching Assistant):`, `(Student):`, `(Classmate):`.
- Insert parenthetical stage directions, emotion cues, or action cues. NEVER write things like `(curiously)`, `(writing notes)`, `(interrupting)`, `(puzzled)`, `(voice-over)`.
