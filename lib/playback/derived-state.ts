 ...
 * from the raw state variables scattered across the classroom UI.
 *
 * Centralises all "what is happening now?" derivation logic so that both the
 * lecture view and the live-discussion (roundtable) view consume a single,
 * consistent view object instead of re-deriving the same conditions inline.
 */

import type { EngineMode, TriggerEvent } from './types';

// ---------------------------------------------------------------------------
// Input: raw state collected from the classroom UI's useState variables
// ---------------------------------------------------------------------------
...
export interface PlaybackRawState {
  ...
  /** Active session type — stays set between agent-loop turns (cleared only by session cleanup). */
  sessionType: string | null;
}

// ---------------------------------------------------------------------------
// Output: a single derived view consumed by the classroom UI
// ---------------------------------------------------------------------------
...
export function computePlaybackView(raw: PlaybackRawState): PlaybackView {
  ...
  // Includes sessionType to bridge the gap between agent-loop turns: a
  // completion event clears chatIsStreaming, but the session is still active
  // until cleanup runs. Without this, bubbleRole briefly falls through to
  ...
  // ---- sourceText (without userMessage — the UI overlays that locally) ----
