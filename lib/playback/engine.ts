...
import type { Scene, Action, SpeechAction, DiscussionAction } from '@shikshasetu/dsl';
import type {
  EngineMode,
  TopicState,
 ...
 * Intentionally low: mixed text often contains punctuation, numbers, and
 * short Latin fragments.
 */
...
export class PlaybackEngine {
  ...
  pause(): void {
    if (this.mode === 'playing') {
      ...
      this.setMode('paused');
      // Freeze TTS — but skip if waiting on a proactive discussion card (no active speech)
      ...
      // Caller is responsible for aborting any in-flight discussion request
    } else {
  ...
  resume(): void {
    ...
    if (this.currentTopicState === 'pending') {
      ...
      // Waiting on the proactive discussion card — just resume mode, don't touch audio
      this.setMode('playing');
    } else {
      // Resume lecture
      this.setMode('playing');
      if (this.browserTTSPausedChunks.length > 0) {
        ...
        // Audio is paused — resume it; onEnded will call processNext
  ...
  /** User clicks "Join" on the proactive discussion card → save cursor → live */
  ...
  /** User clicks "Skip" on the proactive discussion card → consumed → processNext */
  skipDiscussion(): void {
    if (this.currentTrigger) {
      this.markDiscussionConsumed(this.currentTrigger.id);
      this.currentTrigger = null;
  ...
  private async processNext(generation: number = this.playbackGeneration): Promise<void> {
    ...
    switch (action.type) {
      case 'speech': {
        ...
        // in @/lib/choreography. Cancelled on pause; resume() calls processNext
        // directly.
      ...
      case 'discussion': {
        ...
        // Delay before showing the proactive discussion card (allows previous
        // speech to finish naturally)
  ...
  private splitIntoChunks(text: string): string[] {
    // Split on sentence-ending punctuation (Latin + CJK + Devanagari danda) and newlines
    const chunks = text
      .split(/(?<=[.!?。！？।\n])\s*/)
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
    ...
    if (!voiceFound) {
      ...
      const cjkRatio =
        chunkText.length > 0
          ...
          : 0;
      utterance.lang =
        devanagariRatio > CJK_LANG_THRESHOLD
          ? 'hi-IN'
          : cjkRatio > CJK_LANG_THRESHOLD
            ? 'zh-CN'
            : 'en-US';
    }
