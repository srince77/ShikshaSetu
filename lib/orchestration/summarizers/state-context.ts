import type { OrchestrationStoreState } from '../types';
 ...
 */
export function buildStateContext(storeState: OrchestrationStoreState): string {
  ...
  if (currentSceneId) {
    ...
    if (currentScene) {
      ...
      // student has finished. Hydration of `quizResults` happens client-side;
      // absent here means the student has not submitted (or the active scene
      // is not the quiz that owns the results).
        ...
        if (hasGradedResults && quizResults) {
          ...
          lines.push(
            [
              ...
              '- If the student asks for help on a SPECIFIC question (by number, by option, or by quoting the stem), you MAY ask a single Socratic question or clarify a concept WITHOUT naming the correct option. Otherwise stay encouraging and meta ("Which question is tripping you up?" / "Take it one question at a time").',
