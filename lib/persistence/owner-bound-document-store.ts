/**
 * Wraps the tenant-agnostic `PgDocumentStore` with ownership enforcement
 * sourced from `stage_meta`: every mutating operation locks (or claims) the
 * `stage_meta` row for the target stage inside the same transaction as the
 * document write, so a foreign write is refused atomically rather than
 * TOCTOU-raced against a separate check.
 *
 * This is the one seam through which routes and agent tools alike should
 * touch course documents once an owner is known — never `PgDocumentStore`
 * directly, which has no idea `stage_meta` exists.
 */
import type { Scene, Stage } from '@/lib/contracts/scene';
import {
  PgDocumentStore,
  type DocumentFolder,
  type DocumentFolderStore,
  type DocumentStore,
...
export interface OwnerBoundDocumentStoreOptions {
  ...
  mutationFence?: (queryable: DocQueryable) => Promise<void>;
}
...
function queryableFor(connection: Pick<PoolClientLike, 'query'>): DocQueryable {
  return {
  ...
  implements DocumentStore<TScene, TStage>, DocumentFolderStore, StageFreshnessManifestStore
{
  constructor(
    ...
    private readonly queryable: DocQueryable,
  ...
  saveDocument(doc: MaicDocument<TScene, TStage>): Promise<void> {
    return this.tagged({ stageId: doc.stage.id, mode: 'create' }, () => this.inner.saveDocument(doc));
  }
  ...
  async deleteDocument(stageId: string): Promise<void> {
    await this.tagged({ stageId, mode: 'delete' }, () =>
      this.runTransaction(async (queryable) => {
        ...
        await queryable.query('UPDATE document_stages SET folder_id = NULL WHERE id = $1', [stageId]);
      }),
    );
  }
  ...
  moveDocumentToFolder(stageId: string, folderId: string): Promise<boolean> {
    return this.tagged({ stageId, mode: 'mutate' }, () => this.inner.moveDocumentToFolder(stageId, folderId));
  }
  ...
  deleteFolder(id: string, mode: 'ungroup' | 'remove'): Promise<{ removedStageIds: string[] } | null> {
  ...
  setStageFolder(stageId: string, folderId: string | null): Promise<boolean> {
    return this.tagged({ stageId, mode: 'mutate' }, () => this.inner.setStageFolder(stageId, folderId));
  }
}
