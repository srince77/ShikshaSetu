 ...
 *
 * A single engine both online (streaming, future chat-driven edits) and
 * offline (playback) paths share. Two execution modes:
 * - Fire-and-forget: spotlight, laser — dispatch and return immediately
 * - Synchronous: speech, whiteboard, discussion — await completion
 *
 * Keeps the reference app's typed-discriminated-union-plus-switch-dispatch
 * architecture. Ported down to what this phase has real infrastructure for:
 * whiteboard CRUD (lib/action/whiteboard-api.ts) instead of the full
 * scene/element/navigation/mode Stage API, and no media-generation task
 * polling for `play_video` (that pipeline is built separately) — a video
 * element with a resolved src just plays directly.
 */
...
import { createWhiteboardAPI, type CourseStore } from '@/lib/action/whiteboard-api';
import { useCanvasStore } from '@/lib/store/canvas';
import { useWhiteboardHistoryStore } from '@/lib/store/whiteboard-history';
import type { AudioPlayer } from '@/lib/playback/audio-player';
import {
...
export class ActionEngine {
  private stageAPI: ReturnType<typeof createWhiteboardAPI>;
  ...
  constructor(
    ...
    this.stageAPI = createWhiteboardAPI(courseStore);
  ...
  resetPlaybackVisualState(): void {
    ...
    const wb = this.stageAPI.get();
    if (wb.success && wb.data) {
      this.stageAPI.update({ elements: [] }, wb.data.id);
    }
  }
  ...
  /**
   * Play a video element directly (no media-generation task polling — that
   * pipeline is built separately; a video with no resolvable src today will
   * simply hit the safety timeout below rather than blocking playback).
   */
  ...
  private async executeWbDrawLatex(
    ...
    let html: string;
    try {
      html = katex.renderToString(action.latex, {
        throwOnError: false,
        displayMode: true,
        output: 'html',
      });
  ...
  private async executeWbEditCode(
    ...
    const elementResult = this.stageAPI.getElement(action.elementId, wb.data.id);
  ...
  private async executeWbDelete(
    ...
    this.stageAPI.deleteElement(action.elementId, wb.data.id);
  ...
  private async executeWbClear(options: ActionExecutionOptions = {}): Promise<void> {
    ...
    // Save snapshot before AI clear (mirrors the UI's manual clear)
